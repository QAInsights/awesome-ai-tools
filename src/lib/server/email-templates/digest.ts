...
export function renderDigest({ userName, tools, unsubscribeUrl }: DigestInput) {
    ...
    const htmlTools = tools.map(tool => `
        <article style="padding:20px 0;border-top:1px solid #333">
            <h2 style="margin:0 0 6px;font-size:20px"><a href="https://ai.dosa.dev/tools/${escapeHtml(tool.slug)}" style="color:#e2c48a">${escapeHtml(tool.name)}</a></h2>
    ...
    const text = [
        ...
        ...tools.flatMap(tool => [
            '',
            `${tool.name} — updated ${tool.lastUpdated}`,
            tool.description,
