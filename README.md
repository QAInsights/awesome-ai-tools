...
| **[Orca](https://www.onorca.dev/)** | Stably AI | Open-source (MIT) "agent development environment" (ADE) for running a fleet of CLI coding agents in parallel — Claude Code, Codex, OpenCode, Pi, Grok, Cursor, Copilot, and 20+ more, each in its own isolated git worktree; fan one prompt across several agents and merge the winner, Ghostty-class terminal splits, embedded Chromium with Design Mode, inline diff annotations sent back to the agent, native GitHub/Linear task boards, SSH worktrees, an `orca` CLI for scripting, and an iOS/Android companion for steering agents from your phone; macOS/Windows/Linux via Homebrew or AUR; [GitHub](https://github.com/stablyai/orca) |
| **[Conductor](https://www.conductor.build/)** | Conductor (YC) | Mac app for running Claude Code, Codex, Cursor, and OpenCode agents in parallel — each task gets its own git worktree workspace with branch, terminal, diff viewer, setup/run scripts, checks, and PR flow; start work from GitHub or Linear issues, review and merge in-app; free local tier reuses your existing subscriptions, Pro adds cloud workspaces, Multiplayer shared workspaces, and the Conductor API |
| **[BrowserSkill](https://github.com/Tencent/BrowserSkill)** | Tencent | Open-source (MIT) CLI (`bsk`) + Chrome/Edge extension that lets Cursor, Claude Code, Codex, OpenClaw, CodeBuddy, Pi, Hermes Agent, DeepSeek Harness, or any shell-capable agent drive your already-logged-in browser in a separate visible Agent Window — explicit tab borrowing, built-in human-in-the-loop handoff for captchas and logins, full-page screenshots, `bsk install-skill` for one-step skill install, and `bsk doctor` diagnostics; macOS/Linux/Windows; [GitHub](https://github.com/Tencent/BrowserSkill) |
| **[Agent Reach](https://github.com/Panniantong/Agent-Reach)** | Panniantong | Open-source (MIT) installer/router (`agent-reach`) that gives Claude Code, Cursor, OpenClaw, Windsurf, or any command-line agent working internet access — clean-Markdown web reading, YouTube transcripts and search, GitHub, RSS, free semantic web search, plus cookie- or browser-session-unlocked Twitter/X, Reddit, LinkedIn, Bilibili, XiaoHongShu, Facebook, and Instagram through primary + fallback backends; one-line agent-driven install, `--safe` check-only mode, and `agent-reach doctor` health checks; [GitHub](https://github.com/Panniantong/Agent-Reach) |
