...
| **[TWZRD Agent Intel](https://intel.twzrd.xyz)** | TWZRD | Trust scoring for Solana AI agent wallets. Verify agent identity before x402 USDC micropayments. MCP server (streamable-HTTP): `{"mcpServers":{"twzrd-agent-intel":{"url":"https://intel.twzrd.xyz/mcp"}}}` — free `score_agent` and `preflight_check` tools, paid `get_trust_receipt` via HTTP 402. |
