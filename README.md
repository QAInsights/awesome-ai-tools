...
| **[Kolkrabbi](https://github.com/onembyte/kolkrabbi)** | onembyte | Go single-binary terminal agent (`kolk`) with chat, code and orchestrated-agent modes. One effort dial — low, medium, high, max — selects which model runs the turn and how wide the agent work goes, rather than how many thinking tokens one model spends. Runs any OpenRouter model, a local model on Ollama, vLLM or any OpenAI-compatible URL, or your own Claude and ChatGPT subscriptions by driving each vendor's own CLI as a subprocess without holding a credential. Every call is appended to a local JSONL with cost, latency and your own 1–5 ratings; no telemetry. Apache-2.0.; [GitHub](https://github.com/onembyte/kolkrabbi) |
